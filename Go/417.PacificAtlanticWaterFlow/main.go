...
func pacificAtlantic(heights [][]int) [][]int {
	...
	atlantic := make(map[[2]int]struct{})
	pacific := make(map[[2]int]struct{})
	dirs := [4][2]int{{0, 1}, {0, -1}, {1, 0}, {-1, 0}}
	var ocean map[[2]int]struct{}
	var bfs func(int, int, int)
	bfs = func(row, col, prev int) {
		if heights[row][col] < prev {
			return
		}
		if _, inOcean := ocean[[2]int{row, col}]; inOcean {
			return
		}
		val := heights[row][col]
		ocean[[2]int{row, col}] = struct{}{}
	...
	for row := range m {
		...
		ocean = atlantic
		bfs(row, lastCol, heights[row][lastCol])
	}
	for col := range n {
		ocean = pacific
		bfs(0, col, heights[0][col])
		ocean = atlantic
		bfs(lastRow, col, heights[lastRow][col])
	}

	res := [][]int{}
	for coord := range pacific {
		if _, ok := atlantic[coord]; ok {
...
func main() {
	...
	for i, test := range tests {
		...
		failed := false
		for _, coord1 := range actual {
			found := false
			for _, coord2 := range test.expected {
				if slices.Equal(coord1, coord2) {
					found = true
					break
				}
			}
		...
		for _, coord1 := range test.expected {
			found := false
			for _, coord2 := range actual {
				if slices.Equal(coord1, coord2) {
					found = true
					break
				}
